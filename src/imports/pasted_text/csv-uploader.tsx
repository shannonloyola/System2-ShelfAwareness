  ...
  const handleFile = useCallback(
    async (file: File) => {
      if (!file.name.endsWith(".csv")) {
        const msg = `"${file.name}" is not a CSV file.`;
        setErrorMsg(msg);
  ...
  return (
    <div className="w-full space-y-4 font-sans">
      ...
      >
        ...
        {uploadState === "parsing" && (
          <p className="text-sm text-blue-600 animate-pulse">Parsing {fileName}…</p>
        )}
      ...
      {/* Warnings */}
      {warnings.length > 0 && (
        <details className="rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-3 text-xs text-yellow-800">
      ...
      {preview.length > 0 && (
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          ...
          {totalRows > previewLimit && (
            <p className="px-4 py-2 text-xs text-gray-400 border-t border-gray-200">
