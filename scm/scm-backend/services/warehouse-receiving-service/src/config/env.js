...
export const env = {
  ...
  port: Number(process.env.SCM_WAREHOUSE_RECEIVING_PORT || 4005),
  databaseUrl: (
    process.env.SCM_WAREHOUSE_RECEIVING_SUPABASE_FULFILLMENT_DATABASE_URL ||
    process.env.SCM_WAREHOUSE_RECEIVING_FULFILLMENT_DATABASE_URL ||
    ""
  ).trim(),
  dbSsl: parseBoolean(process.env.SCM_WAREHOUSE_RECEIVING_DB_SSL, true),

  // Identity Project (for Auth)
  supabaseUrl: (
    process.env.NEXT_PUBLIC_SCM_WAREHOUSE_RECEIVING_SUPABASE_URL ||
    ""
  ).trim(),
  supabaseAnonKey: (
    process.env.NEXT_PUBLIC_SCM_WAREHOUSE_RECEIVING_SUPABASE_ANON_KEY ||
    ""
  ).trim(),
  supabaseServiceRoleKey: (process.env.SCM_WAREHOUSE_RECEIVING_SUPABASE_SERVICE_ROLE_KEY || "").trim(),

  // Supply Chain Project (Suppliers, Procurement)
  scmSupabaseUrl: (
    process.env.NEXT_PUBLIC_SCM_WAREHOUSE_RECEIVING_SUPABASE_SUPPLY_CHAIN_URL ||
    "https://wbktqkjdsqrvqxxtitsg.supabase.co"
  ).trim(),
  scmSupabaseAnonKey: (
    process.env.NEXT_PUBLIC_SCM_WAREHOUSE_RECEIVING_SUPABASE_SUPPLY_CHAIN_ANON_KEY ||
  ...
  fulfillmentSupabaseUrl: (
    process.env.NEXT_PUBLIC_SCM_WAREHOUSE_RECEIVING_SUPABASE_FULFILLMENT_URL ||
    "https://dkqvbyewfyzfmisyisgs.supabase.co"
  ).trim(),
  fulfillmentSupabaseAnonKey: (
    process.env.NEXT_PUBLIC_SCM_WAREHOUSE_RECEIVING_SUPABASE_FULFILLMENT_ANON_KEY ||
  ...
  fulfillmentSupabaseServiceRoleKey: (process.env.SCM_WAREHOUSE_RECEIVING_SUPABASE_FULFILLMENT_SERVICE_ROLE_KEY || "").trim(),
};
